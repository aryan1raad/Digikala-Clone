...
const SearchedPage = () => {
  ...
  return (
    <div className={styles["Cont"]}>
      <div className={styles["Cont__InnerCont"]}>
        <div className={styles["Cont__innerFlex"]}>

          <section className={styles["LeftSec"]}>
            <div className={styles["LeftSec__sortingCont"]} dir="rtl">
              <div className={styles["LeftSec__sortingCont--sorting"]}>
            ...
            <div className={styles["ShowingGrid"]}>
              ...
              {showingItems.map((prd) => {
                return (
                  <div key={prd.id} className={styles["product--div"]}>
                    <Link to={`/product/${prd.id}`} className={styles["product--a"]}>
                      <div className={styles["product--inner"]}>
                        <article className={styles["product__article"]}>
                          <div className={styles["product__article--inner"]}>
                            <div></div>
                          </div>
                          ...
                          <div style={{ width: '100%' }}>
                            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
                              <div dir="rtl" style={{ marginLeft: 'auto', marginRight: 'auto', display: 'flex' }}>
                                <div className={styles["product__article--imgCont"]}>
                                  <img src={prd.img} alt={prd.title} />
                                </div>
                                <div className={styles["product--colors"]}>
                                  {prd.colors.map((color: string) => {
                                    if (color) return (
                                      <span className={styles["product__article--circleSpan"]} style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: `${color}` }}></span>
                                    )
                                  })}
                                </div>
                              </div>
                            </div>
                            <div className={styles["product__article--properties"]}>

                              <div style={{ height: '23px' }}></div>
                              <div>
                                <h3 className={styles["product__article--title"]}>{prd.title}</h3>
                              </div>
                              <div className={styles["product__article--ersalAndstar"]}>
                              ...
                              <div>
                                <div className={styles["product__article--pricePercent"]}>
                                  <span className={styles["product__article--price"]} dir='ltr'>
                                    <img src="/IMGS/PishnahadIMGs/SVGs/toman.png" style={{ width: '11px' }} />{prd.price}
                                  </span >
                                  {prd.percent && <div className={styles["product__article--percent"]}>
                                    {prd.percent}%
                                  </div>}
                                </div>
                                <span className={styles["product__article--prevPrice"]}>
                                  {prd.prevPrice}
                                </span>
                              </div>
